...
let callbackURLString = "TwitterClient://oauth"
...
class TwitterClient: BDBOAuth1RequestOperationManager {
        ...
        TwitterClient.shared?.getFullTweet(id: original_tweet_id, includeMyRetweet: "t") { (tweet, error) in
            if let fetchedTweet = tweet {
                ...
                if let retweet_id = fetchedTweet.currentUserID {
                    TwitterClient.shared?.destroyTweet(id: retweet_id)
                }
            } else {
                print("untweeting failed!! it was unable to fetch the original tweet")
            }
        }
    }
    ...
    func destroyTweet(id:String) {
        ...
        post("1.1/statuses/destroy.json", parameters: params, success: { (operation:AFHTTPRequestOperation, response: Any) in
            ...
            print("No tweet was destroyed, fail!")
        }
    }
    ...
    func login() {
        ...
        fetchRequestToken(withPath: requestTokenURL, method: "GET", callbackURL: URL(string:callbackURLString), scope: nil, success: { (requestToken: BDBOAuth1Credential!) in
